import { formatBytes, formatIsoTimestamp, formatRelativeTime } from "../lib/format";
...
function EventRow({ event, onSelectAgent }) {
  return (
    <button type="button" className="inspector-event-row" onClick={() => event.agent && onSelectAgent(event.agent)}>
      <span className="timeline-event-time">{formatRelativeTime(event.ts)}</span>
      <div>
  ...
  if (!selectedNode) {
    return (
      <section className="panel inspector-panel">
        ...
        <p className="panel-empty-copy">Click a node in the graph to inspect it.</p>
      ...
      <div className="inspector-body">
              ...
              {selectedNode.data.outputs.length ? (
                <div className="inspector-output-list">
                  {selectedNode.data.outputs.map((output) => (
                    <div key={output.path} className="inspector-output-row">
                      <div>
        ...
        <div className="inspector-activity">
          ...
          {recentEvents.length ? (
            ...
            <span className="inspector-muted">No activity for this node yet</span>
          )}
        </div>
      </div>
    </section>
